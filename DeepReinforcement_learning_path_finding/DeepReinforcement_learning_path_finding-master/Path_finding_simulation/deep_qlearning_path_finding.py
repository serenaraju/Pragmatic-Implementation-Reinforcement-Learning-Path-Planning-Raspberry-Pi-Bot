...
def check_best_path(memory):
    ...
    best_path_s = list(best_path.split())
    #best_path_s = np.array(best_path_s)
    print("Best Path is", best_path_s)
    dir_json = {"array" : best_path_s}
    dir_obj = Loco()
    dir_obj.write(dir_json)
    '''best_path = np.array(best_path.split())
    file = open("best_path", "wb")
    # save array to the file
    np.save(file,best_path)'''
...
if __name__ == "__main__":
    ...
    Button(frame3, text="START TRAINING DEEP Q LEARNING AGENT",bg="red2", command=trainDQN).pack()
    Button(frame4, text="TEST DEEP Q LEARNING AGENT", command=resetEV3Query).pack(side=LEFT)
